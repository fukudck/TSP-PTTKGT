...
void ACOGraph::importTSPDataSet(ifstream& file)
{
	TSP::importTSPDataSet(file);
	distances.resize(cities_num, std::vector<double>(cities_num, 0.0));
	pheromones.resize(cities_num, std::vector<double>(cities_num, 1.0));
}

int ACOGraph::size() const
{
    return cities_num;
}


Ant::Ant(const ACOGraph& graph) : graph(graph), visited(graph.size(), false) {}
